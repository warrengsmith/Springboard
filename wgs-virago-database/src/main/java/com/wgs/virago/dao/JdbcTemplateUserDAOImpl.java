...
public class JdbcTemplateUserDAOImpl extends NamedParameterJdbcDaoSupport implements UserDAO {
	...
	private static final class UserMapper implements RowMapper<User> {
		...
		public User mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
			...
			user.setName(resultSet.getString("USER_NM"));
			return user;
		}
	}
	...
	public User findUserById(final int userId) {
		User user = null;
		try {
			user = getNamedParameterJdbcTemplate().queryForObject("select USER_ID, USER_NM from USER where USER_ID = :userId",
					new MapSqlParameterSource("userId", userId), new UserMapper());
	...
	public User findUserByName(final String name) {
		User user = null;
		try {
			user = getNamedParameterJdbcTemplate().queryForObject("select USER_ID, USER_NM from USER where USER_NM = :name",
					new MapSqlParameterSource("name", name), new UserMapper());
		} catch (DataAccessException e) { // NOPMD by kahwgs on 5/22/14 12:34 PM
			// do nothing
		}
		return user;
	 ...
	 * 
	 * @see
	 * com.usg.ssg1.common.dao.UserDAO#updateUser(com.usg.ssg1.common.dto.User)
	 */
	@Override
